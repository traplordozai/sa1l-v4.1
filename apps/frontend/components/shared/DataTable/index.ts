        ...
        <td colSpan={displayColumns.length + 1} className="py-10 text-center">
          <div className="flex flex-col items-center justify-center">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {Object.keys(filters).length > 0 ? "Try adjusting your filters or search terms." : "No data available."}
            </p>
      ...
      <div className="border-b border-gray-200 bg-white px-4 py-5 sm:px-6">
        <div className="-ml-4 -mt-2 flex flex-wrap items-center justify-between sm:flex-nowrap">
          ...
          <div className="ml-4 mt-2 flex-shrink-0 flex space-x-2">
            ...
            {/* Export options */}
            {onExport && <DataTableExport onExport={onExport} />}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <p className="text-sm text-gray-700">
                Showing <span className="font-medium">{processedData.length > 0 ? pageIndex * pageSize + 1 : 0}</span>{" "}
                to <span className="font-medium">{Math.min((pageIndex + 1) * pageSize, processedData.length)}</span> of{" "}
            ...
            <Pagination currentPage={pageIndex + 1} totalPages={totalPages} onPageChange={handlePageChange} />
          </div>
        </div>
      )}
    </div>
